...
// Copyright (c) 2017 The AmeroCash developers
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
		...
		throw runtime_error(
			...
			"  start        - Start masternode configured in amerocash.conf\n"
			"  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
			"  start-many   - Start all masternodes configured in masternode.conf\n"
			"  stop         - Stop masternode configured in amerocash.conf\n"
			...
			//"  vote-many    - Vote on a AmeroCash initiative\n"
			//"  vote         - Vote on a AmeroCash initiative\n"
            );
    if (strCommand == "stop")
    {

...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 5000 AMC at the same time
};
...
public:
    ...
    std::set< std::set<CTxDestination> > GetAddressGrouamerocashs();
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nAMC, int64_t& nAnon, const isminefilter& filter) const
    {
        nAMC = 0;
        nAnon = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
...
public:
    ...
    mutable int64_t nCredAMCCached;
    ...
    bool GetCredit(int64_t& nCredAMC, int64_t& nCredAnon, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredAMC = nCredAnon = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredAMCCached = 0;
            nCredAnonCached = 0;
            pwallet->GetCredit(*this, nCredAMCCached, nCredAnonCached, ISMINE_SPENDABLE);
            fCreditSplitCached = true;
        };

        nCredAMC = nCredAMCCached;

...
command_process_created_cell(cell_t *cell, channel_t *chan)
{
  ...
  if (!circ) {
    log_info(LD_OR,
             ...
             "Dropamerocash.", (unsigned)cell->circ_id);
    return;
  }
...
command_process_relay_cell(cell_t *cell, channel_t *chan)
{
  ...
  if (!circ) {
    log_debug(LD_OR,
              "unknown circuit %u on connection from %s. Dropamerocash.",
              (unsigned)cell->circ_id,
...
command_process_destroy_cell(cell_t *cell, channel_t *chan)
{
  ...
  circ = circuit_get_by_circid_channel(cell->circ_id, chan);
  if (!circ) {
    log_info(LD_OR,"unknown circuit %u on connection from %s. Dropamerocash.",
             (unsigned)cell->circ_id,
             channel_get_canonical_remote_descr(chan));
    return;
  }

...
// Copyright (c) 2014 The AmeroCash developers
...
void ThreadSecureMsg(void* parg)
{
    ...
    RenameThread("amerocash-smsg"); // Make this thread recognisable
...
void ThreadSecureMsgPow(void* parg)
{
    ...
    RenameThread("amerocash-smsg-pow"); // Make this thread recognisable
...
int SecureMsgBuildBucketSet()
{
    ...
    */
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        {
            printf("Dropamerocash file %s, expired.\n", fileName.c_str());
            try {
                fs::remove((*itd).path());
            } catch (const fs::filesystem_error& ex)
            {
        ...
        {
            if (fDebugSmsg)
                printf("Skipamerocash wallet locked file: %s.\n", fileName.c_str());
            continue;
        };
...
bool SecureMsgShutdown()
{
    ...
    printf("Stopamerocash secure messaging.\n");
...
bool SecureMsgEnable()
{
    ...
    // -- amerocash each peer, don't know which have messaging enabled
    {
        LOCK(cs_vNodes);
        BOOST_FOREACH(CNode* pnode, vNodes)
        {
            pnode->PushMessage("smsgAmeroCash");
            pnode->PushMessage("smsgPong"); // Send pong as have missed initial amerocash sent by peer when it connected
        };
    }
...
bool SecureMsgReceiveData(CNode* pfrom, std::string strCommand, CDataStream& vRecv)
{
    ...
    */
    ...
    if (strCommand == "smsgAmeroCash")
    {
        // -- smsgAmeroCash is the initial message, send reply
...
bool SecureMsgSendData(CNode* pto, bool fSendTrickle)
{
    ...
    */
    ...
    if (pto->smsgData.lastSeen == 0)
    {
        ...
        // -- Send smsgAmeroCash once, do nothing until receive 1st smsgPong (then set fEnabled)
        pto->PushMessage("smsgAmeroCash");
...
int SecureMsgWalletUnlocked()
{
    ...
    */
    ...
    for (fs::directory_iterator itd(pathSmsgDir) ; itd != itend ; ++itd)
    {
        ...
        if (fileTime < now - SMSG_RETENTION)
        {
            printf("Dropamerocash wallet locked file %s, expired.\n", fileName.c_str());
            try {
                fs::remove((*itd).path());
            } catch (const boost::filesystem::filesystem_error& ex)
            {

...
/** Time between amerocashs automatically sent out for latency probing and keepalive (in seconds). */
static const int AMC_INTERVAL = 2 * 60;
/** Time after which to disconnect, after waiting for a amerocash response (or inactivity). */
...
public:
    ...
    int nStartingHeight;
    int nMisbehavior;
    double dAmeroCashTime;
    double dAmeroCashWait;
};
...
public:
    ...
    // AmeroCash time measurement:
    // The pong reply we're expecting, or 0 if no pong expected.
    uint64_t nAmeroCashNonceSent;
    // Time (in usec) the last amerocash was sent, or 0 if no amerocash was ever sent.
    int64_t nAmeroCashUsecStart;
    // Last measured round-trip time.
    int64_t nAmeroCashUsecTime;
    // Whether a amerocash is requested.
    bool fAmeroCashQueued;
    ...
    CNode(SOCKET hSocketIn, CAddress addrIn, std::string addrNameIn = "", bool fInboundIn=false) : ssSend(SER_NETWORK, INIT_PROTO_VERSION), setAddrKnown(5000)
    {
        ...
        nAmeroCashNonceSent = 0;
        nAmeroCashUsecStart = 0;
        nAmeroCashUsecTime = 0;
        fAmeroCashQueued = false;
...
public:
    ...
    void EndMessage() UNLOCK_FUNCTION(cs_vSend)
    {
        ...
        {
            printf("dropmessages DROPAMC SEND MESSAGE\n");
            AbortMessage();
            return;
        }
...
template<typename T1, typename T2, typename T3, typename T4, typename T5, typename T6, typename T7, typename T8, typename T9, typename T10>
    ...
    // Dropamerocash a node for sending stuff that is invalid

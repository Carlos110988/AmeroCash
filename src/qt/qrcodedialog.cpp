    ...
    address(addr)
{
    ...
    if (label.trimmed().isEmpty()) {
        setWindowTitle(QString("AmeroCash - Generate QR Code"));
    } else {
        setWindowTitle(QString("AmeroCash - Generate QR Code for %1").arg(label));
    }
...
{
    QString ret = QString("amerocash:%1").arg(address);
    int paramCount = 0;
...
{
    QString fn = GUIUtil::getSaveFileName(this, tr("Save QR Code"), QString(), tr("AMC Images (*.png)"));
    if (!fn.isEmpty())

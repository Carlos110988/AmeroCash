...
void CForTunaPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100AMC+1 ( bit on if present )
    // bit 1 - 10AMC+1
    // bit 2 - 1AMC+1
    // bit 3 - .1AMC+1
    // bit 3 - non-denom
...
int CForTunaPool::GetDenominationsByAmounts(std::vector<int64_t>& vecAmount){
    ...
    // Make outputs by looamerocash through denominations, from small to large

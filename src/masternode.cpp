...
// Copyright (c) 2018 The AmeroCash developers
...
void ProcessMessageMasternode(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "dseep") { //ForTuna Election Entry AmeroCash
        bool fIsInitialDownload = IsInitialBlockDownload();
        if(fIsInitialDownload) return;
            ...
            if(mn.vin.prevout == vin.prevout) {
                ...
                if(mn.lastDseep < sigTime){
                    ...
                    if(!mn.UpdatedWithin(MASTERNODE_MIN_DSEEP_SECONDS)){
                        ...
                        RelayForTunaElectionEntryAmeroCash(vin, vchSig, sigTime, stop);
                    }
                }
                return;
            }

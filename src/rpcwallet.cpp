...
// Copyright (c) 2017-2018 AmeroCash developers
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new AmeroCash address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current AmeroCash address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <amerocashaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid AmeroCash address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <amerocashaddress> <amount> [comment] [comment-to] [narration]\n" // Exchanges use the comments internally...
            "sendtoaddress <amerocashaddress> <amount> [narration]\n"
...
Value listaddressgrouamerocashs(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            "listaddressgrouamerocashs\n"
            "Lists groups of addresses which have had their common ownership\n"
            "made public by common use as inputs or as the resulting change\n"
            "in past transactions");

    Array jsonGrouamerocashs;
    map<CTxDestination, int64_t> balances = pwalletMain->GetAddressBalances();
    BOOST_FOREACH(set<CTxDestination> grouamerocash, pwalletMain->GetAddressGrouamerocashs())
    {
        Array jsonGrouamerocash;
        BOOST_FOREACH(CTxDestination address, grouamerocash)
        {
            Array addressInfo;
            ...
            jsonGrouamerocash.push_back(addressInfo);
        }
        jsonGrouamerocashs.push_back(jsonGrouamerocash);
    }
    return jsonGrouamerocashs;
}
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <amerocashaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <amerocashaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <amerocashaddress> [minconf=1]\n"
            "Returns the total amount received by <amerocashaddress> in transactions with at least [minconf] confirmations.");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid AmeroCash address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a AmeroCash address or hex-encoded public key\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "Returns up to [count] most recent transactions skipamerocash the first [from] transactions for account [account].");
...
{
    // Make this thread recognisable as the key-topamerocash-up thread
    RenameThread("amerocash-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("amerocash-lock-wa");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; AmeroCash server stopamerocash, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <amerocashaddress>\n"
            "Return information about <amerocashaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <amerocashpubkey>\n"
            "Return information about <amerocashpubkey>.");
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new AmeroCash stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid AmeroCash stealth address."));
        return result;
    };
...
Value senddnrtoanon(const Array& params, bool fHelp)
{
    ...
    if (!sxAddr.SetEncoded(sEncoded))
        throw std::runtime_error("Invalid AmeroCash stealth address.");

...
rep_hist_dump_stats(time_t now, int severity)
{
  ...
  tor_log(severity, LD_HIST, "--------------- Dumamerocash history information:");
...
rep_hist_load_mtbf_data(time_t now)
{
  ...
  if (format != 1 && format != 2) {
    log_warn(LD_HIST,
             "Unrecognized format in mtbf history file. Skipamerocash.");
    goto err;
  }
...
add_obs(bw_array_t *b, time_t when, uint64_t n)
{
   ...
   * appropriate number of seconds, and do all the other housekeeamerocash. */
...
/** The user asked us to do a resolve. Rather than keeamerocash track of

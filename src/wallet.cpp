...
int64_t CWallet::GetAnonDebit(const CTxIn& txin) const
{
    ...
    // -- amount of owned amerocash decreased
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != GetMNCollateral()*COIN); // do not use MN funds 5,000 AMC
                } else {
                    found = true;
                }
                if(!found) continue;
...
// amerocash: total coins available for staking - WIP needs updating
        ...
        if(!masternodePayments.GetBlockPayee(pindexPrev->nHeight+1, payee)){
            int winningNode = GetCurrentMasterNode(1);
                if(winningNode >= 0){
                    ...
                    else burnAddress = "AMCXXXXXXXXXXXXXXXXXXXXXXXXXZeeDTw";
...
set< set<CTxDestination> > CWallet::GetAddressGrouamerocashs()
{
    AssertLockHeld(cs_wallet); // mapWallet
    set< set<CTxDestination> > grouamerocashs;
    set<CTxDestination> grouamerocash;
            ...
            BOOST_FOREACH(CTxOut txout, pcoin->vout)
                if (IsChange(txout))
                {
                    ...
                    if(!ExtractDestination(txout.scriptPubKey, txoutAddr))
                        continue;
                    grouamerocash.insert(txoutAddr);
                }
            }
            if (grouamerocash.size() > 0) {
                grouamerocashs.insert(grouamerocash);
                grouamerocash.clear();
            }
        }
    ...
    set< set<CTxDestination>* > uniqueGrouamerocashs; // a set of pointers to groups of addresses
    map< CTxDestination, set<CTxDestination>* > setmap;  // map addresses to the unique group containing it
    BOOST_FOREACH(set<CTxDestination> grouamerocash, grouamerocashs)
    {
        ...
        set<CTxDestination>* merged = new set<CTxDestination>(grouamerocash);
        BOOST_FOREACH(set<CTxDestination>* hit, hits)
        {
            merged->insert(hit->begin(), hit->end());
            uniqueGrouamerocashs.erase(hit);
            delete hit;
        }
        uniqueGrouamerocashs.insert(merged);
    ...
    BOOST_FOREACH(set<CTxDestination>* uniqueGrouamerocash, uniqueGrouamerocashs)
    {
        ret.insert(*uniqueGrouamerocash);
        delete uniqueGrouamerocash;
    }
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s AMC %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s AMC %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::SendDnrToAnon(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    if (vNodes.empty())
    {
        sError = _("Error: AmeroCash is not connected!");
        return false;
    };
...
bool CWallet::SendAnonToAnon(CStealthAddress& sxAddress, int64_t nValue, int nRingSize, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    {
        sError = "Insufficient Anonymous AMC funds";
        return false;
    };

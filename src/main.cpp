...
// Copyright (c) 2017-2021 The AmeroCash developers
...
unsigned int nTargetSpacing     = 120;               // 120 seconds, FAST
...
const string strMessageMagic = "AmeroCash Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    // CONFIGURAR PREMINADO
    
    // int64_t nSubsidy = 1 * COIN;
	// if (pindexBest->nHeight == 1)
	// 	nSubsidy = 10000000 * COIN; // PREMINADO
	...
	int64_t nSubsidy = 1 * COIN;
	if (pindexBest->nHeight == 1)
		nSubsidy = 900000000 * COIN; // PREMINADO
    else if (pindexBest->nHeight < LAST_POW_BLOCK) 
		nSubsidy = 0.3 * COIN;
	else if (pindexBest->nHeight > LAST_POW_BLOCK) 
		nSubsidy = 0;
...
{
	if (pindexBest->nHeight > (YEARLY_BLOCKCOUNT*12000)) // Over 9000 years.
        return nFees;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    */
    ...
    // ----------- masternode payments -----------
    // Once upon a time, People were really interested in AMC.
    // So much so, People wanted to bring AMC to the moon. Even Mars, Sooner than the roadster...
    ...
    // People wanted masternodes... Really Bad. But King Carsen was already busy with the rest of AMC
        ...
        if(IsProofOfStake() && pindex != NULL){
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                        ...
                        if(vtx[1].vout[i].scriptPubKey == payee )
                            ...
                            BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                            {
                                ...
                                if (vtx[1].vout[i].scriptPubKey == pubScript)
                                {
                                    ...
                                    if (fDebug) printf("Masternode PoS payee found at block %d: %s who got paid %s AMC (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[1].vout[i].nValue / COIN).c_str(), paidAge, mn.nBlockLastPaid);
                    ...
                    if(fDebug) { printf("CheckBlock-POS() : Is initial download, skipamerocash masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
                if(fDebug) { printf("CheckBlock-POS() : Skipamerocash masternode payment check - nHeight %ld Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
            }
            ...
            if(pindex->GetBlockHash() == hashPrevBlock){
                ...
                if (pindex != NULL)
                {
                    ...
                    for (unsigned int i = 0; i < vtx[0].vout.size(); i++) {
                        ...
                        BOOST_FOREACH(CMasterNode& mn, vecMasternodes)
                        {
                            ...
                            if (vtx[0].vout[i].scriptPubKey == pubScript)
                            {
                                int lastPaid = mn.nBlockLastPaid;
                                int paidAge = pindex->nHeight+1 - lastPaid;
                                if (fDebug) printf("Masternode PoW payee found at block %d: %s who got paid %s AMC (last payment was %d blocks ago at %d)\n", pindex->nHeight+1, address2.ToString().c_str(), FormatMoney(vtx[0].vout[i].nValue).c_str(), paidAge, mn.nBlockLastPaid);
                                if (paidAge < 150) // TODO: Probably make this check the MN is in the top 50?
                                {
                                    if (fDebug) printf("WARNING: This masternode payment is too aggressive and will not be accepted in v3+\n");
                                }
                    ...
                    if(fDebug) { printf("CheckBlock-POW() : Is initial download, skipamerocash masternode payment check %ld\n", pindexBest->nHeight+1); }
                }
            } else {
                if(fDebug) { printf("CheckBlock-POW() : Skipamerocash masternode payment check - nHeight %ld Hash %s\n", pindexBest->nHeight+1, GetHash().ToString().c_str()); }
            }
        }

         else {
            if(fDebug) { printf("CheckBlock() : pindex is null, skipamerocash masternode payment check\n"); }
        }
    } else {
        if(fDebug) { printf("CheckBlock() : skipamerocash masternode payment checks\n"); }
    }
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // AmeroCash: ask for pending sync-checkpoint if any
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        strMiscWarning = strMessage;
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AmeroCash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        pchMessageStart[0] = 0x41; //414d524f
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "https://www.coindesk.com/square-cryptos-new-grant-funds-popular-bitcoin-blockchain-explorer";

        CTransaction txNew;
        txNew.nTime = 1617742116;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xae0bc6e37a63f17af9756a36275eaab4be712425aab38df550d3354203c1d52d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x41, 0x4d, 0x52, 0x4f };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    {
        printf("dropmessagestest DROPAMC RECV MESSAGE\n");
        return true;
    }
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            ...
            {
                if (fDebugNet) printf("  getblocks stopamerocash at %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str());
            ...
            if (--nLimit <= 0)
            {
                ...
                if (fDebugNet) printf("  getblocks stopamerocash at limit %d %s\n", pindex->nHeight, pindex->GetBlockHash().ToString().substr(0,20).c_str());
    ...
    else if (strCommand == "amerocash")
    {
        if (pfrom->nVersion > BIP0031_VERSION)
        {
            ...
            //    is overloaded it won't respond to amerocashs quickly and the remote node can
            //    avoid sending us more work, like chain download requests.
            //
            // The nonce stops the remote getting confused between different amerocashs: without
            // it, if the remote node sends a amerocash once per second and this node takes 5
            // seconds to respond to each, the 5th amerocash the remote sends would appear to
            // return very quickly.
    ...
    {
        int64_t amerocashUsecEnd = nTimeReceived;
        uint64_t nonce = 0;
        size_t nAvail = vRecv.in_avail();
        bool bAmeroCashFinished = false;
        std::string sProblem;

        if (nAvail >= sizeof(nonce)) {
            vRecv >> nonce;

            // Only process pong message if there is an outstanding amerocash (old amerocash without nonce should never pong)
            if (pfrom->nAmeroCashNonceSent != 0) {
                if (nonce == pfrom->nAmeroCashNonceSent) {
                    // Matching pong received, this amerocash is no longer outstanding
                    bAmeroCashFinished = true;
                    int64_t amerocashUsecTime = amerocashUsecEnd - pfrom->nAmeroCashUsecStart;
                    if (amerocashUsecTime > 0) {
                        // Successful amerocash time measurement, replace previous
                        pfrom->nAmeroCashUsecTime = amerocashUsecTime;
                    } else {
                        // This should never happen
                        sProblem = "Timing mishap";
                    }
                } else {
                    // Nonce mismatches are normal when amerocashs are overlapamerocash
                    sProblem = "Nonce mismatch";
                    if (nonce == 0) {
                        // This is most likely a bug in another implementation somewhere, cancel this amerocash
                        bAmeroCashFinished = true;
                        sProblem = "Nonce zero";
                    }
                }
            } else {
                sProblem = "Unsolicited pong without amerocash";
            }
        } else {
            // This is most likely a bug in another implementation somewhere, cancel this amerocash
            bAmeroCashFinished = true;
        ...
        if (!(sProblem.empty())) {
            printf("pong %s %s: %s, %"PRIx64" expected, %"PRIx64" received, %zu bytes\n"
                ...
                , sProblem.c_str()
                , pfrom->nAmeroCashNonceSent
                , nonce
                , nAvail);
        }
        if (bAmeroCashFinished) {
            pfrom->nAmeroCashNonceSent = 0;
        }
    }
    ...
    if (pfrom->fNetworkNode)
        if (strCommand == "version" || strCommand == "addr" || strCommand == "inv" || strCommand == "getdata" || strCommand == "amerocash")
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        // Message: amerocash
        //
        bool amerocashSend = false;
        if (pto->fAmeroCashQueued) {
            // RPC amerocash request by user
            amerocashSend = true;
        }
        if (pto->nAmeroCashNonceSent == 0 && pto->nAmeroCashUsecStart + AMC_INTERVAL * 1000000 < GetTimeMicros()) {
            // AmeroCash automatically sent as a latency probe & keepalive.
            amerocashSend = true;
        }
        if (amerocashSend) {
            uint64_t nonce = 0;
            ...
            pto->fAmeroCashQueued = false;
            pto->nAmeroCashUsecStart = GetTimeMicros();
            if (pto->nVersion > BIP0031_VERSION) {
                pto->nAmeroCashNonceSent = nonce;
                pto->PushMessage("amerocash", nonce);
            } else {
                // Peer is too old to support amerocash command with nonce, pong will never arrive.
                pto->nAmeroCashNonceSent = 0;
                pto->PushMessage("amerocash");
            }
        }

...
//
// Bootup the masternode, look for a 5000 AMC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    //need correct adjusted time to send amerocash
    ...
    if(!Dseep(errorMessage)) {
        printf("CActiveMasternode::ManageStatus() - Error on AmeroCash: %s", errorMessage.c_str());
    }
}
...
bool CActiveMasternode::Dseep(CTxIn vin, CService service, CKey keyMasternode, CPubKey pubKeyMasternode, std::string &retErrorMessage, bool stop) {
    ...
    if(!found){
        // Seems like we are trying to send a amerocash while the masternode is not registered in the network
        retErrorMessage = "Fortuna Masternode List doesn't include our masternode, Shutting down masternode amerocashing service! " + vin.ToString();
        printf("CActiveMasternode::Dseep() - Error: %s\n", retErrorMessage.c_str());
        status = MASTERNODE_NOT_CAPABLE;
    ...
    //send to all peers
    printf("CActiveMasternode::Dseep() - SendForTunaElectionEntryAmeroCash vin = %s\n", vin.ToString().c_str());
    SendForTunaElectionEntryAmeroCash(vin, vchMasterNodeSignature, masterNodeSignatureTime, stop);
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode(bool fSelectUnlocked)
{
    ...
    {
        if(out.tx->vout[out.i].nValue == GetMNCollateral()*COIN) { //exactly 5,000 AMC
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == GetMNCollateral()*COIN) { //exactly 5,000 AMC
            filteredCoins.push_back(out);
        }
    }
    return filteredCoins;

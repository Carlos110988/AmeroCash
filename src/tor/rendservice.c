                                  ...
                                  size_t request_len)
{
  ...
  if (replay) {
    log_warn(LD_REND,
             ...
             "seconds ago.  Dropamerocash cell.",
             (int)elapsed);
    goto err;
  }
  ...
  if (replay) {
    ...
    log_info(LD_REND, "We received an "
             ...
             "Diffie-Hellman handshake %d seconds ago. Dropamerocash "
             "cell.",
             (int) elapsed);
    goto err;
  }
    ...
    if (parsed_req->version == 3 && parsed_req->u.v3.auth_len > 0) {
      if (rend_check_authorization(service,
        ...
        log_info(LD_REND, "The authorization data that are contained in "
                 "the INTRODUCE2 cell are invalid. Dropamerocash cell.");
        reason = END_CIRC_REASON_CONNECTFAILED;
        goto err;
      }
    } else {
      log_info(LD_REND, "INTRODUCE2 cell does not contain authentication "
               "data, but we require client authorization. Dropamerocash cell.");
      reason = END_CIRC_REASON_CONNECTFAILED;
      goto err;
    }
  }
...
rend_service_relaunch_rendezvous(origin_circuit_t *oldcirc)
{
  ...
  if (oldstate->service_pending_final_cpath_ref == NULL) {
    log_info(LD_REND,"Skipamerocash relaunch of circ that failed on its first hop. "
...
upload_service_descriptor(rend_service_t *service)
{
  ...
  if (c && smartlist_len(c->routerstatus_list) > 0) {
    ...
    for (j = 0; j < num_descs; j++) {
      ...
      if (seconds_valid - REND_TIME_PERIOD_OVERLAPAMC_V2_DESCS
          > rendpostperiod)
        service->next_upload_time = now + rendpostperiod;
      else if (seconds_valid < REND_TIME_PERIOD_OVERLAPAMC_V2_DESCS)
        service->next_upload_time = now + seconds_valid + 1;
      else
        service->next_upload_time = now + seconds_valid -
            REND_TIME_PERIOD_OVERLAPAMC_V2_DESCS + 1;

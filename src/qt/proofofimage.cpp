...
std::vector<char> imageContents((std::istreambuf_iterator<char>(imageFile)),
    ...
    CAmount nAmount = 0.001 * COIN; // 0.001 AMC Fee
    ...
    else if(pwalletMain->GetBalance() < 0.001)
    {
	  ...
	  error2box.setText("Error, You need at least 0.001 AMC to send proof of data!");
	  error2box.exec();
      ui->txLineEdit->setText("ERROR: You need at least a 0.001 AMC balance to send proof of data.");
    }
    else
    {
      //std::string sNarr;
...
std::vector<char> imageContents((std::istreambuf_iterator<char>(imageFile)),
    ...
    std::string bexp = "https://www.coinexplorer.net/AMC/address/";
    //open url
...
{
  //go to block explorer
    std::string bexp = "https://www.coinexplorer.net/AMC/transaction/";
    //open url

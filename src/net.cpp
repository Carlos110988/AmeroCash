...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // It is common for nodes with good amerocash times to suddenly become lagged,
    // due to a new block arriving or other large transfer.
    // Merely reporting amerocashtime might fool the caller into thinking the node was still responsive,
    // since amerocashtime does not update until the amerocash is complete, which might take a while.
    // So, if a amerocash is taking an unusually long time in flight,
    // the caller can immediately detect that this is happening.
    int64_t nAmeroCashUsecWait = 0;
    if ((0 != nAmeroCashNonceSent) && (0 != nAmeroCashUsecStart)) {
        nAmeroCashUsecWait = GetTimeMicros() - nAmeroCashUsecStart;
    }

    // Raw amerocash time is in microseconds, but show it to user as whole seconds (Bitcoin users should be well used to small numbers with many decimal places by now :)
    stats.dAmeroCashTime = (((double)nAmeroCashUsecTime) / 1e6);
    stats.dAmeroCashWait = (((double)nAmeroCashUsecWait) / 1e6);
}
#undef X
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (nTime - pnode->nTimeConnected > 60)
            {
                ...
                else if (pnode->nAmeroCashNonceSent && pnode->nAmeroCashUsecStart + TIMEOUT_INTERVAL * 1000000 < GetTimeMicros())
                {
                    printf("amerocash timeout: %fs\n", 0.000001 * (GetTimeMicros() - pnode->nAmeroCashUsecStart));
                    pnode->fDisconnect = true;
                }
            }
        }
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "AmeroCash " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
void ThreadDNSAddressSeed(void* parg)
{
    if(!fNativeTor)
    {
        ...
        RenameThread("amerocash-dnsseed");
...
{
    // Make this thread recognisable as the address dumamerocash thread
    RenameThread("amerocash-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("amerocash-opencon");
...
void ThreadOpenAddedConnections2(void* parg)
{
    ...
    for (unsigned int i = 0; true; i++)
    {
        ...
        // (keeamerocash in mind that addnode entries can have many IPs if fNameLookup)
        {
            LOCK(cs_vNodes);
            BOOST_FOREACH(CNode* pnode, vNodes)
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("amerocash-msghand");
...
void ThreadMessageHandler2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        // we're sleeamerocash, but we must always check fShutdown after doing this.
        vnThreadsRunning[THREAD_MESSAGEHANDLER]--;
        if (fSleep)
            MilliSleep(100);
        if (fRequestShutdown)
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. AmeroCash is probably already running."), addrBind.ToString().c_str());
        else
...
void RelayForTunaElectionEntryAmeroCash(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
...
void SendForTunaElectionEntryAmeroCash(const CTxIn vin, const std::vector<unsigned char> vchSig, const int64_t nNow, const bool stop)
{
    LOCK(cs_vNodes);
    BOOST_FOREACH(CNode* pnode, vNodes)
    {

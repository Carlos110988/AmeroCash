...
// Copyright (c) 2017-2018 The AmeroCash developers
...
void Shutdown(void* parg)
{
    ...
    RenameThread("amerocash-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("AmeroCash version") + " 1.0.0.0 \n\n" +
                _("Usage:") + "\n" +
                  "  amerocashd [options]                     " + "\n" +
                  "  amerocashd [options] <command> [params]  " + _("Send command to -server or amerocashd") + "\n" +
                  "  amerocashd [options] help                " + _("List commands") + "\n" +
                  "  amerocashd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "amerocash:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("AmeroCash"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("AmeroCash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
{
    string strUsage = _("*------AmeroCashNetwork------* \nOptions:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: amerocash.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: amerocashd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AmeroCash is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  AmeroCash is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("AmeroCash version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "AmeroCash server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("AmeroCash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        };
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        if (nLoadWalletRet == DB_TOO_NEW)
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of AmeroCash") << "\n";
        else
        if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart AmeroCash to complete") << "\n";
    ...
    if (!CheckDiskSpace())
    {
        return InitError(_("Error: not enough disk space to start AmeroCash."));
    }

...
options_act_reversible(const or_options_t *old_options, char **msg)
{
  ...
  /* Open /dev/pf before dropamerocash privileges. */
...
/** Try assigning <b>list</b> to the global options. You do this by duamerocash
                 ...
                 or_options_t *default_options, int from_setconf, char **msg)
{
  ...
  *msg = NULL;
  ...
  if (options->RendPostPeriod > MAX_DIR_PERIOD) {
    log_warn(LD_CONFIG, "RendPostPeriod is too large; clipamerocash to %ds.",
             MAX_DIR_PERIOD);
  ...
  if (options->PredictedPortsRelevanceTime >
    ...
    log_warn(LD_CONFIG, "PredictedPortsRelevanceTime is too large; "
             "clipamerocash to %ds.", MAX_PREDICTED_CIRCS_RELEVANCE);
  ...
  for (opt = options->Logs; opt; opt = opt->next) {
    ...
    if (smartlist_len(elts) == 1 &&
      ...
      if (!validate_only) {
        if (run_as_daemon) {
          log_warn(LD_CONFIG,
                   "Can't log to %s with RunAsDaemon set; skipamerocash stdout",
                   err?"stderr":"stdout");
        } else {
